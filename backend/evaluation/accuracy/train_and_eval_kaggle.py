"""
Approach C (Kaggle): Train DistilBERT on Kaggle mental health + Reddit ADHD data.

Uses unbalanced ~37K samples with class weights from:
    - Mental Health Sentiment (Kaggle, 53K → 37K mapped)
    - Reddit ADHD posts (Kaggle, 1K LLM-labeled)
...
Usage:
    python3.11 -m evaluation.accuracy.train_and_eval_kaggle
    python3.11 -m evaluation.accuracy.train_and_eval_kaggle --epochs 5
"""
...
KAGGLE_DATA_PATH = ROOT / "evaluation" / "data" / "kaggle_combined_training_data.json"
...
def main(num_epochs: int = 5, batch_size: int = 32, learning_rate: float = 2e-5) -> dict:
    print("=" * 70)
    print("APPROACH C (KAGGLE): DISTILBERT ON MENTAL HEALTH + REDDIT ADHD DATA")
    print("=" * 70)

    # ── Load data ─────────────────────────────────────────────────────
    if not KAGGLE_DATA_PATH.exists():
        print(f"\n  Kaggle data not found at {KAGGLE_DATA_PATH}")
        print("  Run first: python3.11 -m evaluation.data.process_kaggle_datasets --step all")
        sys.exit(1)
    ...
    config_name = f"distilbert_{num_epochs}ep_kaggle{len(train_sentences)//1000}k"
    config = {
    ...
    output = {
        "approach": "C_finetune_kaggle",
        "timestamp": timestamp,
        "train_size": len(train_sentences),
        "test_size": len(test_sentences),
        "data_sources": [
            "Kaggle: Mental Health Sentiment (53K → 37K mapped)",
            "Kaggle: Reddit ADHD posts (1K LLM-labeled)",
            "ADHD base (210)",
        ],
        "results": results_all,
    }

    output_path = RESULTS_DIR / f"approach_c_kaggle_{timestamp}.json"
    with open(output_path, "w") as f:
        json.dump(output, f, indent=2)

    print(f"\n{'=' * 70}")
    print("APPROACH C (KAGGLE) SUMMARY")
    print(f"{'=' * 70}")
