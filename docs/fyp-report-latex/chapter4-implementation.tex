...
Each coaching prompt is augmented with the emotion classification, SenticNet affective dimensions, relevant memories from Mem0, and the current screen activity classification. This context is injected as structured key-value pairs so the LLM need not infer information the system already knows.
...
def build_prompt(
    ...
    senticnet: SenticNetResult,
    memories: list[Memory],
    screen: ScreenContext,
) -> str:
    context_block = (
        f"[Emotion: {emotion.label} ({emotion.confidence:.2f})]\n"
        f"[Sentic: P={senticnet.pleasantness:.2f} "
        f"A={senticnet.attention:.2f}]\n"
        f"[Screen: {screen.app} - {screen.category}]\n"
        f"[Memories: {format_memories(memories)}]\n"
    )
    return SYSTEM_PROMPT + context_block + user_message
\end{lstlisting}
...
The production classifier achieves 86\% accuracy with mean confidence 0.76 (vs 0.33 in Approach A). Per-class F1 scores are reported in \cref{tab:per-class-f1}.
...
The lowest-performing categories (\emph{frustrated} and \emph{overwhelmed}, both F1 = 0.74) share semantic overlap in expressions of negative valence and high task difficulty. The 0.76 mean confidence enables threshold-based decision-making in the intervention engine.
...
The SenticNet pipeline \cite{cambria2024, cambria2016} provides a complementary affective analysis channel, enriching coaching responses with continuous-valued dimensions (pleasantness, attention, sensitivity, aptitude) and semantic associations from commonsense knowledge. The implementation interfaces with 13 API endpoints organised into a four-tier cascade:
...
The persistent memory system uses Mem0 \cite{chhikara2025} backed by PostgreSQL with \texttt{pgvector}. It stores episodic memories from coaching interactions and emotion-state transitions, and maintains an evolving user profile via exponential moving averages ($\alpha = 0.1$). Each memory is encoded into a 768-dimensional embedding using \texttt{all-mpnet-base-v2} \cite{reimers2019}---the same model used by the emotion classifier---and stored with metadata including timestamp, emotion classification, and a relevance decay factor (30-day half-life). Retrieval uses cosine similarity via \texttt{pgvector}'s \texttt{<=>} operator, returning the top-3 most relevant memories for context injection (\cref{subsec:context-injection}). Evaluation on a benchmark of 100 query-memory pairs shows 95\% top-1 relevance.
...
The intervention engine implements a Just-In-Time Adaptive Intervention (JITAI) framework. Decision points are triggered by the screen monitor detecting distraction; tailoring variables are the user's emotional state, distraction duration, and historical success rate; intervention options follow a four-tier escalation: \emph{nudge} (subtle menu bar animation), \emph{popup} (non-blocking notification with coaching message), \emph{overlay} (semi-transparent screen overlay with current task), and \emph{block} (opt-in temporary application restriction).

Intervention selection uses Thompson Sampling with Beta-distributed priors ($\alpha_0 = \beta_0 = 1$) to balance exploitation of effective interventions with exploration of underused strategies. Each intervention type is modelled as a Bernoulli bandit arm, where success is defined as the user returning to productive activity within five minutes. Emotional context modulates tone: frustrated users receive empathetic nudges, disengaged users receive energetic prompts. The Thompson Sampling state is persisted to PostgreSQL between sessions, reaching consistent selections after approximately 20 interventions.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}lXX@{}}
    ...
    Memory constraints &
      ...
      Load-on-demand LLM with weak reference cache; peak AI memory below 2.5\,GB \\
    \addlinespace
