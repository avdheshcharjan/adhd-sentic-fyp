...
\Cref{fig:app-notch-flow} presents the complete notch-based UX flow, showing the five interaction states from dormant monitoring through alert-level intervention delivery.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{screenshot-notch-flow.jpg}
  \caption{Notch-based UX flow showing the five interaction states: dormant, ambient, glanceable, expanded, and alert.}
  \label{fig:app-notch-flow}
\end{figure}

\subsection{Chat Interface and Brain Dump}

The chat interface provides conversational ADHD coaching powered by the on-device Qwen3-4B model. Users can toggle between standard mode and extended reasoning (\texttt{/think}) mode, which generates longer, more thoughtful responses at the cost of higher latency. The brain dump modal allows rapid capture of unstructured thoughts, which are automatically tagged with emotion labels and stored in semantic memory. \Cref{fig:app-braindump} shows both modals.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{screenshot-braindump-vent.jpg}
  \caption{Brain dump and vent chat modals for rapid thought capture and conversational coaching.}
  \label{fig:app-braindump}
\end{figure}

\subsection{Intervention Popup}

When the JITAI engine determines that an intervention is warranted, a notification popup appears with ADHD-friendly visual tokens (\cref{fig:app-intervention}). Each intervention includes an acknowledgement of the user's current state, a concrete suggestion, and action buttons. User responses feed back into the Thompson Sampling bandit to personalise future intervention timing and type.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{screenshot-intervention.jpg}
  \caption{Notch expanded state showing a JITAI intervention with action buttons.}
  \label{fig:app-intervention}
\end{figure}

\subsection{React Dashboard}

The React dashboard (served via Vite) provides a comprehensive view of productivity metrics, emotion timelines, and session insights (\cref{fig:app-dashboard}). The dashboard is accessible at \texttt{http://localhost:5173} and communicates with the backend via the REST API documented in \cref{app:api-docs}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{screenshot-dashboard.jpg}
  \caption{React dashboard displaying emotion timeline, productivity metrics, and session insights.}
  \label{fig:app-dashboard}
\end{figure}

\subsection{Settings Panel}

The settings panel allows configuration of monitoring intervals, intervention thresholds, model parameters, and Google Calendar OAuth integration (\cref{fig:app-settings}). Changes are persisted to the backend configuration and take effect immediately.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{screenshot-settings.jpg}
  \caption{Settings panel showing permission configuration options.}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}l cccccc@{}}
    ...
    \midrule
    Joyful      & \textbf{7} & 0 & 0 & 0 & 1 & 0 \\
    Focused     & 0 & \textbf{7} & 0 & 1 & 0 & 0 \\
    Frustrated  & 0 & 0 & \textbf{6} & 0 & 1 & 1 \\
    Anxious     & 0 & 1 & 0 & \textbf{6} & 0 & 1 \\
    Disengaged  & 0 & 0 & 1 & 0 & \textbf{7} & 0 \\
    Overwhelmed & 0 & 0 & 1 & 1 & 0 & \textbf{6} \\
    \bottomrule
  \end{tabular}
\end{table}
