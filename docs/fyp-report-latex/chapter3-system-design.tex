...
%     L[SenticNet Cloud]
%     M[Whoop API v2]
%     N[OpenAI GPT-4o-mini + Embeddings]
%   end
%   A --> Backend
%   B --> Backend
%   C --> Backend
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[usercomp, right=0.6cm of react] (openclaw) {Telegram Bot\\[-1pt]{\tiny Native Python}};
    ...
    % ── Data Layer ──
    \node[datacomp, below=1.4cm of memory, xshift=1.0cm] (pg) {PostgreSQL\\+ pgvector};
    ...
    \node[extcomp, below=0.5cm of whoopapi] (gpt) {OpenAI\\[-1pt]{\tiny GPT-4o-mini + Embeddings}};
    ...
    \begin{scope}[on background layer]
      ...
      \node[fit=(senticcloud)(whoopapi)(gpt),
    ...
    % ── Arrows: User -> Backend ──
    \draw[arr] (swift.south) -- ++(0,-0.35) -| (backlayer.north);
    \draw[arr] (react.south) -- ++(0,-0.35) -| (backlayer.north);
    \draw[arr] (openclaw.south) -- ++(0,-0.35) -| (backlayer.north);

    % ── Arrows: Backend -> Data ──
    \draw[arr] (memory.south) -- ++(0,-0.45) -| (pg.north);
    \draw[arr] (whoop.south) -- ++(0,-0.45) -| (pg.north);
    ...
    \draw[arr, dashed] (memory.east) -- ++(1.8,0) |- (gpt.west);

  \end{tikzpicture}
  \caption{Three-layer system architecture of the ADHD Second Brain. The User Layer comprises a SwiftUI menu bar application, a React dashboard, and a Telegram bot interface. The Backend Layer hosts the FastAPI server (\texttt{localhost:8420}) with its constituent modules (${\sim}500$\,MB RAM $+$ ${\sim}2.3$\,GB GPU). The Data Layer uses PostgreSQL with pgvector for persistent and vector-indexed storage. Dashed arrows indicate external service calls.}
  \label{fig:system_architecture}
\end{figure}

\textbf{User Layer.} The primary interface is a native macOS menu bar application built in SwiftUI ($\sim$25\,MB RAM), which polls screen metadata via the Accessibility API every two to three seconds and forwards it to the Backend Layer over local HTTP. A React dashboard (Vite) provides historical analytics and emotional trend visualisations, while a Telegram bot provides a conversational interface for venting, task planning, and reflective journaling.
...
\textbf{Data Layer.} PostgreSQL with pgvector provides combined relational and vector storage. Activity logs, emotional analyses, and conversation histories are stored relationally, while Mem0-generated embeddings are indexed via pgvector for sub-second semantic search.

\textbf{External Services.} Three external services are used selectively: SenticNet Cloud (thirteen affective computing APIs)~\cite{cambria2024}, Whoop API~v2 (physiological metrics), and the OpenAI API (GPT-4o-mini for Mem0 memory extraction, \texttt{text-embedding-3-small} for memory embeddings).
...
%     direction TB
%     W1[User Message] --> W2[SenticNet Analysis<br>Safety + Emotion + ADHD]
%     W2 --> W3{Safety Gate}
%     W3 -->|Pass| W4[LLM Generation]
%     W3 -->|Critical| W3b[Crisis Response]
%     W4 --> W5[Memory Store]
%   end
% --- END MERMAID ---
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[warmbox, below=of w1] (w2) {SenticNet Analysis\\[-2pt]{\scriptsize Safety + Emotion + ADHD}};
    \node[warmbox, below=of w2] (w3) {Safety Gate\\[-2pt]{\scriptsize Critical? $\to$ crisis path}};
    \node[warmbox, below=of w3] (w4) {Build LLM Context\\[-2pt]{\scriptsize Emotion + memory retrieval}};
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
  ...
  \item \emph{Resource consciousness}: peak AI memory is constrained to below 3\,GB with load-on-demand model instantiation for consumer Apple Silicon hardware.
\end{enumerate}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[tier4, below=of t3] (t4) {%
      ...
      {\footnotesize Personality $\cdot$ Ensemble}};
    ...
    \draw[downarr] (t3.south) -- (t4.north);

    % Emergency arrow
    \draw[emergarr] (t1.east) -- node[above, font=\scriptsize\sffamily, text=red!60!black]{Threshold exceeded} (emerg.west);

    % Annotation for Tier 4 — skipped in lightweight mode
    \draw[decorate, decoration={brace, amplitude=4pt, mirror}, thick, blue!40!black]
      ([xshift=-0.4cm]t4.south west) -- ([xshift=-0.4cm]t4.north west)
      node[midway, left=5pt, font=\scriptsize\sffamily, text=blue!50!black, align=right]
      {Skipped in\\lightweight mode};

  \end{tikzpicture}
  \caption{Four-tier SenticNet orchestration pipeline. Each tier is evaluated sequentially; Tier~1 may short-circuit to an emergency pathway if safety thresholds are exceeded. All four tiers execute in full analysis mode; Tier~4 is skipped only in lightweight mode.}
...
% --- MERMAID BACKUP ---
% flowchart LR
%   A[User Input] --> B[Encode<br>text-embedding-3-small]
%   B --> C[pgvector Search<br>Cosine Similarity]
%   C --> D[Inject into<br>LLM Prompt]
% --- END MERMAID ---
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[pipebox, right=of input] (encode) {Encode\\[-2pt]{\scriptsize text-embedding-\\3-small (OpenAI)}};
    \node[pipebox, right=of encode] (search) {pgvector\\Search\\[-2pt]{\scriptsize Cosine Sim.}};
    \node[pipebox, right=of search] (inject) {Inject into\\LLM Prompt};

    \draw[arr] (input) -- (encode);
    \draw[arr] (encode) -- (search);
    \draw[arr] (search) -- (inject);
  \end{tikzpicture}
  \caption{Semantic memory retrieval pipeline. User input is encoded via OpenAI's \texttt{text-embedding-3-small} model, matched against the pgvector index by cosine similarity, and injected into the LLM prompt.}
...
The user interface comprises two complementary surfaces. The SwiftUI menu bar application provides an always-available, minimal-footprint entry point displaying current focus score, emotional state, and streak duration, with access to the Telegram bot conversational interface. It follows macOS Human Interface Guidelines to minimise cognitive overhead---critical for users with ADHD, who are disproportionately affected by interface complexity~\cite{beattie2025}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{2.8cm}|p{3.8cm}p{7cm}}
    ...
    \addlinespace
    On-device LLM & Qwen3-4B via MLX & 4-bit quantisation fits within 2.3\,GB GPU budget; MLX exploits Apple Silicon unified memory for zero-copy inference; 37.1\,tok/s throughput enables interactive conversation \\
    \addlinespace
    ...
    \addlinespace
    Sentence encoder & all-mpnet-base-v2 & State-of-the-art sentence embeddings on STS benchmarks~\cite{reimers2019}; used for emotion classification and activity classification. Memory retrieval uses OpenAI \texttt{text-embedding-3-small} via Mem0 \\
    \addlinespace
