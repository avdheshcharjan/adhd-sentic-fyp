...
\Cref{tab:per-class-f1} presents per-class F1 scores for the production SetFit model on 50 held-out test sentences. Joyful and focused achieved perfect F1 (1.00) due to distinctive lexical markers. Disengaged and overwhelmed achieved the lowest (0.75 and 0.82 respectively), reflecting genuine semantic overlap between these low-arousal and high-arousal negative states.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    Joyful      & 1.00 & 1.00 & 1.00 \\
    Focused     & 1.00 & 1.00 & 1.00 \\
    Frustrated  & 0.73 & 0.89 & 0.80 \\
    Anxious     & 1.00 & 0.67 & 0.80 \\
    Disengaged  & 0.75 & 0.75 & 0.75 \\
    Overwhelmed & 0.78 & 0.88 & 0.82 \\
    \midrule
    Macro Average & 0.88 & 0.86 & 0.862 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{fig:confusion-matrix} presents the confusion matrix for the production SetFit model. Two primary confusion patterns emerge: an anxious--frustrated/disengaged/overwhelmed axis (clinically expected, as the boundary between worry-driven states and adjacent negative emotions is ambiguous even for human annotators) and a disengaged--overwhelmed axis (low-energy and high-load states share surface-level lexical markers). Joyful and focused achieve perfect classification. The diagonal dominance---43 correct out of 50---confirms reliable discrimination across all six categories.

% --- MERMAID BACKUP ---
% ```mermaid
% graph LR
%   subgraph "Confusion Matrix (SetFit Production, 86%)"
%     A["Predicted vs Actual 6x6 heatmap"]
%     B["joyful:     8, 0, 0, 0, 0, 0"]
%     C["focused:    0, 8, 0, 0, 0, 0"]
%     D["frustrated: 0, 0, 8, 0, 1, 0"]
%     E["anxious:    0, 0, 1, 6, 1, 1"]
%     F["disengaged: 0, 0, 1, 0, 6, 1"]
%     G["overwhelmed:0, 0, 1, 0, 0, 7"]
%   end
% ```
% --- END MERMAID ---
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[
        ...
        (0,5) [8]  (1,5) [0]  (2,5) [0]  (3,5) [0]  (4,5) [0]  (5,5) [0]
        (0,4) [0]  (1,4) [8]  (2,4) [0]  (3,4) [0]  (4,4) [0]  (5,4) [0]
        (0,3) [0]  (1,3) [0]  (2,3) [8]  (3,3) [0]  (4,3) [1]  (5,3) [0]
        (0,2) [0]  (1,2) [0]  (2,2) [1]  (3,2) [6]  (4,2) [1]  (5,2) [1]
        (0,1) [0]  (1,1) [0]  (2,1) [1]  (3,1) [0]  (4,1) [6]  (5,1) [1]
        (0,0) [0]  (1,0) [0]  (2,0) [1]  (3,0) [0]  (4,0) [0]  (5,0) [7]
      };

      % Cell annotations — joyful (y=5)
      \node[font=\small\bfseries, text=white] at (axis cs:0,5) {8};
      \node[font=\small] at (axis cs:1,5) {0};
      \node[font=\small] at (axis cs:2,5) {0};
      \node[font=\small] at (axis cs:3,5) {0};
      \node[font=\small] at (axis cs:4,5) {0};
      \node[font=\small] at (axis cs:5,5) {0};
      % focused (y=4)
      \node[font=\small] at (axis cs:0,4) {0};
      \node[font=\small\bfseries, text=white] at (axis cs:1,4) {8};
      \node[font=\small] at (axis cs:2,4) {0};
      \node[font=\small] at (axis cs:3,4) {0};
      ...
      \node[font=\small\bfseries, text=white] at (axis cs:2,3) {8};
      \node[font=\small] at (axis cs:3,3) {0};
      \node[font=\small] at (axis cs:4,3) {1};
      \node[font=\small] at (axis cs:5,3) {0};
      % anxious (y=2)
      \node[font=\small] at (axis cs:0,2) {0};
      \node[font=\small] at (axis cs:1,2) {0};
      \node[font=\small] at (axis cs:2,2) {1};
      \node[font=\small\bfseries, text=white] at (axis cs:3,2) {6};
      \node[font=\small] at (axis cs:4,2) {1};
      ...
      \node[font=\small\bfseries, text=white] at (axis cs:4,1) {6};
      \node[font=\small] at (axis cs:5,1) {1};
      % overwhelmed (y=0)
      \node[font=\small] at (axis cs:0,0) {0};
      \node[font=\small] at (axis cs:1,0) {0};
      \node[font=\small] at (axis cs:2,0) {1};
      \node[font=\small] at (axis cs:3,0) {0};
      \node[font=\small] at (axis cs:4,0) {0};
      \node[font=\small\bfseries, text=white] at (axis cs:5,0) {7};
    \end{axis}
  \end{tikzpicture}
  \caption{Confusion matrix for the production SetFit emotion classifier evaluated on 50 held-out test sentences (43 correct, 86\% accuracy). Diagonal entries indicate correct classifications; off-diagonal entries indicate misclassifications. The primary confusion axis lies in the anxious category, which is misclassified across frustrated, disengaged, and overwhelmed.}
...
The persistent memory system, built on Mem0 with pgvector \cite{chhikara2025}, exhibited a mean store latency of 6,454~ms (P95: 9,313~ms), dominated by the embedding forward pass and PostgreSQL WAL flush. Retrieval latency averaged 306~ms (P95: 344~ms), with occasional outliers from sequential scans on small result sets. The top-1 hit rate was 89\% across 100 queries over 20 user profiles, with Hit@3 reaching 98\% and Hit@5 reaching 99\%. The two misses involved semantically adjacent but distinct content (e.g., a ``feeling emotional about client feedback'' query retrieving related but not target memories). RSS was stable at approximately 556~MB.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{2.5cm}|p{2.8cm}|p{4.2cm}|p{3.2cm}|}
    ...
    \hline
    Obj 4: Screen + JITAI + Memory & Real-time classification, $\geq$90\% Hit@1 & 549 titles/s, 7 rules, 89\% Hit@1 (Hit@3: 98\%) & Achieved \\[4pt]
    \hline
    Obj 5: Resource efficiency & Minimal battery, $<$6\,GB & 21,314\,mJ, $<$3\,GB, $\sim$1.4\%/hr & Exceeded (2$\times$ memory margin) \\
    \hline
  \end{tabular}
\end{table}
